...
from ..service import amocrm_daigo
...
class LeadCreationAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        amocrm_moloko.handle_deal(deal_id, "Лид")
        return Response(status=status.HTTP_200_OK)


class UnqualifiedLeadCreationAPIView(APIView):
    def post(self, request, *args, **kwargs):
        logger.info(f"request_data: {request.data}\n"
                    f"request_time: {datetime.now()}\n"
                    f"lead_type: Unqualified\n")
        deal_id = request.data.get("leads[status][0][id]", [""])[0]
        amocrm_moloko.handle_deal(deal_id, "Неквалифицированный")
        return Response(status=status.HTTP_200_OK)
...
class LeadTransferAPIView(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        amocrm_daigo.send_lead_to_amocrm(
            validated_contact,
            validated_deal
        )
        return Response(status=status.HTTP_200_OK)
